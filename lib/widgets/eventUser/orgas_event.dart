  ...
  builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
    if (snapshot.connectionState == ConnectionState.done) {
      if (snapshot.hasData && snapshot.data!.exists) {
        List<dynamic> listEvent = snapshot.data!.get('eventOrganise');
        if (listEvent.isEmpty) {
          return const Center(child: Text('Aucun événement organisé'));
        }
                ...
                if (snapshot.connectionState == ConnectionState.done) {
                  if (snapshot.hasData && snapshot.data!.exists) {
                    ...
                    return ListTile(
                      ...
                      onTap: () {
                        Navigator.of(context).pushNamed('/ShowOptions', arguments: {'id': '${listEvent[index]}', 'nom': '${event['nom']}'});
                      },
